...
public class SpringWebMvcUtil
{
  ...
  public static boolean isAsyncRequest(HttpServletRequest request, Object handler)
  {
    ...
    if (!isAsync)
    {
      isAsync = ServletWebUtil.isAjaxRequest(request);
    }
    return isAsync;
  }
  ...
  public static void writeHttpMessage(HttpServletRequest request, HttpServletResponse response, Object responseBody, MediaType... responseMediaTypes) throws IOException
  {
    ...
    AssertUtil.notNull(request, "Parameter 'request' must be required!");
    AssertUtil.notNull(response, "Parameter 'response' must be required!");
    AssertUtil.state(!response.isCommitted(), "The http response has been committed!");
    if (responseBody != null)
    {
      ...
      if (CollectionUtils.isEmpty(acceptedMediaTypes) && !ArrayUtil.isEmpty(responseMediaTypes))
      { //如果参数指定了响应MediaType
